...
//        }
        ...
        if ($this->lastPage <= $side * 2) {
            ...
            $block = $this->getUrlRange(1, $side *2);
        } elseif ($this->currentPage > ($this->lastPage - $side)) {
            $block = $this->getUrlRange($this->lastPage - $side * 2 + 1 , $this->lastPage);
        } else {            
            $block = $this->getUrlRange($this->currentPage - $side, $this->currentPage + $side-1);

        }
        
        $html = '';
        
        $html .= $this->getUrlLinks($block);
    ...
    public function render()
    {
        if ($this->hasPages()) {
            if ($this->simple) {
                ...
                return sprintf(
                    '%s<div id="pagination"><div>%s %s %s %s %s %s</div></div>',
                    $this->css(),
                    $this->home(),    
                    $this->prev(),
                    $this->getLinks(),
